...
export const mockPatient_Garcia_NoSocial = {
  ...
  lastName: "García",
  dni: "11222334",
  telephone: "3884665016",
  birthDate: "1987-06-24",
  membershipNumber: "",
  user: {
    userId: 206,
    email: "marto.garcia2@gmail.com",
    createdAt: "2024-05-15T14:30:00",
    active: true,
    role: "Patient",
  },
...
export const doctorScheduleMock = [
  {
    ...
    startTime: format(
      setMinutes(setHours(currentWeekStart, 9), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(currentWeekStart, 9), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
  ...
  },
  {
    ...
    startTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 2), 10), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 2), 10), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
  ...
  },
  {
    ...
    startTime: format(
      setMinutes(setHours(addDays(lastWeekStart, 4), 11), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(lastWeekStart, 4), 11), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
  ...
  },
  {
    ...
    startTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 15), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 15), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
  ...
  {
    shiftId: 106,
    // Jueves de esta semana 16:00 PM (Otro paciente - Martinez)
    startTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 16), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 16), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    reason: "Consulta primera vez.",
    status: mockShiftStatus.attended,
  ...
  {
    shiftId: 107,
    // Jueves de esta semana 17:00 PM (Otro paciente - Martinez)
    startTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 16), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 17), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    reason: "Consulta primera vez.",
  ...
  },
  {
    ...
    // Jueves de esta semana 17:00 PM (Otro paciente - Martinez)
    startTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 17), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(currentWeekStart, 3), 18), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    reason: "Consulta primera vez.",
    status: mockShiftStatus.pending,
  ...
  },
  {
    ...
    startTime: format(
      setMinutes(setHours(nextWeekStart, 9), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(nextWeekStart, 9), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
...
export const patientScheduleMock = [
  ...
  },
  {
    shiftId: 107,
    startTime: format(
      setMinutes(setHours(currentWeekStart, 12), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(currentWeekStart, 12), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    reason: "Dolor persistente en rodilla derecha.",
    status: mockShiftStatus.attended,
  ...
  // Turno con OTRO médico (Dra. Torres) - Semana pasada
  {
    shiftId: 201,
    startTime: format(
      setMinutes(setHours(addDays(lastWeekStart, 1), 16), 0),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    endTime: format(
      setMinutes(setHours(addDays(lastWeekStart, 1), 16), 30),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
...
//       setMinutes(setHours(addDays(lastWeekStart, 4), 11), 15),
//       "yyyy-MM-dd'T'HH:mm:00"
//     ),
//     diagnosis: "Contusión leve en rodilla derecha.",
//     treatment: "Hielo local, reposo deportivo por 7 días.",
//     personalNotes:
//       "Paciente refiere dolor al subir escaleras. Se descarta fractura.",
//     medications: [
//       {
//         prescriptionId: 901,
...
//         issueDate: format(
//           setMinutes(setHours(addDays(lastWeekStart, 4), 11), 20),
//           "yyyy-MM-dd'T'HH:mm:00"
//         ),
//       },
//     ],
//     // RELACIÓN: La consulta conoce a su turno
//     // Usamos .find para que sea robusto y no dependa del índice del array
//     shift: doctorScheduleMock.find((s) => s.shiftId === 103),
//   },
// ];

/**
 * HISTORIAL CLÍNICO COMPLETO DEL PACIENTE GARCIA
 * Turnos asociados (Estado: Atendido):
 * - 103: Urgencia (Caída bici) - Dr. Sanchez
 * - 201: Chequeo Cardiológico - Dra. Torres
 * - 107: Seguimiento Rodilla - Dr. Sanchez
 * - 108: Control final Rodilla - Dr. Sanchez
 */

export const completedConsultationsMock = [
  // 1. Consulta vinculada al turno 103 (La urgencia de la semana pasada)
  {
    consultationId: 501,
    consultationDate: format(
      setMinutes(setHours(addDays(lastWeekStart, 4), 11), 15),
    ...
    treatment: "Reposo deportivo, crioterapia local.",
    personalNotes: "Paciente refiere dolor agudo al flexionar. Se descarta fractura ósea por tacto, pero se sugiere observación.",
    medications: [
      {
        prescriptionId: 901,
        name: "Ibuprofeno 600mg",
        dosage: "1 comprimido cada 8 horas",
        instructions: "Tomar con abundante agua y con el estómago lleno.",
        issueDate: format(setMinutes(setHours(addDays(lastWeekStart, 4), 11), 20), "yyyy-MM-dd'T'HH:mm:00"),
      },
    ],
    shift: patientScheduleMock.find((s) => s.shiftId === 103),
  },

  // 2. Consulta vinculada al turno 201 (Cardiología con Dra. Torres - Semana pasada)
  {
    consultationId: 502,
    consultationDate: format(
      setMinutes(setHours(addDays(lastWeekStart, 1), 16), 20),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    diagnosis: "Examen cardiovascular normal",
    treatment: "Control anual preventivo.",
    personalNotes: "Presión arterial 120/80. Electrocardiograma sin particularidades. Paciente realiza deporte (fútbol/voley) regularmente.",
    medications: [], // Sin medicación
    shift: patientScheduleMock.find((s) => s.shiftId === 201),
  },

  // 3. Consulta vinculada al turno 107 (Seguimiento rodilla - Esta semana)
  {
    consultationId: 503,
    consultationDate: format(
      setMinutes(setHours(currentWeekStart, 12), 15),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    diagnosis: "Tendinitis rotuliana leve",
    treatment: "Sesiones de Kinesiología (10) y ejercicios de fortalecimiento.",
    personalNotes: "El dolor ha disminuido, pero persiste molestia 'punzante' después del ejercicio. Se deriva a Kinesiología.",
    medications: [
        {
        prescriptionId: 902,
        name: "Diclofenac Gel",
        dosage: "Aplicar 3 veces al día",
        instructions: "Masajear la zona afectada suavemente.",
        issueDate: format(setMinutes(setHours(currentWeekStart, 12), 20), "yyyy-MM-dd'T'HH:mm:00"),
      },
    ],
    shift: patientScheduleMock.find((s) => s.shiftId === 107),
  },

  // 4. Consulta vinculada al turno 108 (Control final rodilla - Esta semana)
  // Nota: Asumimos que este turno ocurrió justo después o fue un sobreturno atendido el mismo día
  {
    consultationId: 504,
    consultationDate: format(
      setMinutes(setHours(currentWeekStart, 12), 45),
      "yyyy-MM-dd'T'HH:mm:00"
    ),
    diagnosis: "Evolución favorable",
    treatment: "Alta médica parcial.",
    personalNotes: "Paciente responde bien a la primera sesión de prueba. Se autoriza retorno progresivo a la actividad física.",
    medications: [],
    shift: patientScheduleMock.find((s) => s.shiftId === 108),
  },
];

export const getConsultationByShiftId = (shiftId) => {
  return (
    completedConsultationsMock.find(
      (c) => c.shift && c.shift.shiftId === shiftId
    ) || null
  );
};
...
export const getMockDoctorSchedule = (viewedMondayDate) => {
  return [
    // --- LUNES ---
    {
      shiftId: 101,
      startTime: format(
        setMinutes(setHours(viewedMondayDate, 9), 0),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      endTime: format(
        setMinutes(setHours(viewedMondayDate, 9), 30),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      status: mockShiftStatus.attended,
      patient: mockPatient_Garcia,
    ...
    // --- MIÉRCOLES ---
    {
      shiftId: 102,
      startTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 2), 10), 0),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      endTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 2), 10), 30),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      status: mockShiftStatus.pending,
      patient: mockPatient_Garcia,
    ...
    // --- JUEVES (Otro paciente) ---
    {
      shiftId: 104,
      startTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 3), 15), 0),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      endTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 3), 15), 30),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      status: mockShiftStatus.pending,
      patient: mockPatient_Martinez,
    ...
    // --- VIERNES (Cancelado) ---
    {
      shiftId: 109,
      startTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 4), 16), 0),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      endTime: format(
        setMinutes(setHours(addDays(viewedMondayDate, 4), 16), 30),
        "yyyy-MM-dd'T'HH:mm:00"
      ),
      status: mockShiftStatus.cancelled,
      patient: mockPatient_Garcia,
